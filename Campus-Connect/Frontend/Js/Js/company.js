...
router.post("/login", (req, res) => {
  ...
  db.query("SELECT * FROM companies WHERE email=?", [email], async (err, result) => {
    ...
    if (companyStatus === "pending")
      return res.json({ success: false, message: "Your registration is under review. Please wait for admin approval.", status: "pending" });
    if (companyStatus === "rejected")
      return res.json({ success: false, message: "Your registration was rejected. Contact support@campusconnect.in", status: "rejected" });
    res.json({
...
router.post("/jobs/create", (req, res) => {
  uploadJobDoc.single("job_pdf")(req, res, (uploadErr) => {
    if (uploadErr) return res.json({ success: false, message: uploadErr.message });

    const { company_id, title, type, location, stipend, duration, apply_link,
            description, min_cgpa, allowed_branches, allowed_degrees,
            max_backlogs, eligible_batches } = req.body;
    ...
    // Parse JSON arrays (sent as JSON strings from FormData)
    var branchesStr, degreesStr, batchesStr;
    try {
      var branches = allowed_branches ? JSON.parse(allowed_branches) : [];
      var degrees  = allowed_degrees  ? JSON.parse(allowed_degrees)  : [];
      var batches  = eligible_batches ? JSON.parse(eligible_batches) : [];
      branchesStr  = branches.length > 0 ? JSON.stringify(branches) : null;
      degreesStr   = degrees.length  > 0 ? JSON.stringify(degrees)  : null;
      batchesStr   = batches.length  > 0 ? JSON.stringify(batches)  : null;
    } catch(e) {
      branchesStr = null; degreesStr = null; batchesStr = null;
    }
    ...
    db.query(
      ...
      [company_id, title, type||"internship", location||null, stipend||null,
       duration||null, apply_link||null, description, "active",
       min_cgpa||0, branchesStr, degreesStr, max_backlogs||0, batchesStr, pdfPath],
      (err, result) => {
        if (err) {
          if (pdfFile) fs.unlinkSync(pdfFile.path);
          return res.json({ success: false, message: "DB error: " + err.message });
  ...
  db.query(
    ...
    (err, results) => {
      ...
      try {
        ...
        ws["!cols"] = [
          {wch:22},{wch:30},{wch:20},{wch:8},{wch:14},
          {wch:32},{wch:8},{wch:10},{wch:45},{wch:12},{wch:14}
        ];
      ...
      db.query("UPDATE applications SET status=? WHERE id=?", [status, application_id], (err) => {
        ...
        try {
          ...
          const emailHtml = status === "accepted"
            ? `<div style="font-family:sans-serif;max-width:500px;margin:auto;padding:32px;background:#f9f5ef;border-radius:16px;"><h2 style="color:#2e7d32;">🎉 Congratulations, ${app.full_name}!</h2><p>Your application for <strong>${app.job_title}</strong> at <strong>${app.company_name}</strong> has been <span style="color:#2e7d32;font-weight:700;">ACCEPTED</span>!</p><p style="margin-top:16px;">The company will reach out to you soon. Best of luck!</p><p style="color:#8b7d6b;font-size:12px;margin-top:24px;">— Campus Connect Team</p></div>`
            : `<div style="font-family:sans-serif;max-width:500px;margin:auto;padding:32px;background:#f9f5ef;border-radius:16px;"><h2 style="color:#c62828;">Application Update</h2><p>Dear <strong>${app.full_name}</strong>,</p><p>Your application for <strong>${app.job_title}</strong> at <strong>${app.company_name}</strong> was not selected this time.</p><p style="margin-top:16px;">Keep applying — the right opportunity is out there!</p><p style="color:#8b7d6b;font-size:12px;margin-top:24px;">— Campus Connect Team</p></div>`;
          transporter.sendMail({
            from: `"Campus Connect" <${process.env.EMAIL_USER}>`,
            to: app.student_email,
            subject: status === "accepted" ? `🎉 Accepted — ${app.job_title} at ${app.company_name}` : `Application Update — ${app.job_title} at ${app.company_name}`,
            html: emailHtml
